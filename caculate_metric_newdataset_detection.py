...
def get_right_num(out, patch_mask,threshold=0.1):
    # print(out.shape)
    # print(patch_mask.shape)
    tumor_mask=out[0,1].cpu().detach().numpy()
    patch_mask=patch_mask[0,1].cpu().detach().numpy()
...
def slideCropF1(img, mask,num_class):
    ...
    if img.shape[1] >= 512 and img.shape[2] >= 512:  # H>512,W>512
        f1,precision,recall = normSlideCropF1(img, mask,num_class)
    elif img.shape[1] >= 512 and img.shape[2] < 512:  # H>=512,W<512
        f1,precision,recall = paddingSlideCropF1_1(img, mask,num_class)
    elif img.shape[1] < 512 and img.shape[2] >= 512:  # H<512,W>=512
        f1,precision,recall = paddingSlideCropF1_2(img, mask,num_class)
    if img.shape[1] < 512 and img.shape[2] < 512:  # H<512,w<512
        f1,precision,recall = paddingSlideCropF1_3(img, mask,num_class)
    ...
    for i in range(0, m + 1):  
        for j in range(0, n + 1): 
            ...
            if i < m and j < n:
                ...
                right_num,predict_num,gt_num=get_right_num(out, patch_mask)
...
def paddingSlideCropF1_3(img, mask,num_class):
    ...
    # DoDnet:
    #patch = torch.cat([torch.from_numpy(patch)] * 4, dim=1).cuda() 
    patch = patch.repeat(4, 1, 1, 1).cuda()
    with torch.no_grad():
        out = unet.forward(patch, task_id)
    out=out[:,:num_class]
...
datasets=['HP','MBM']
task_id_dict={'Adipocyte':9,'HP':6,'MBM':8}
channel_dict={'Adipocyte':2,'HP':2,'MBM':2}
...
for dataset in datasets:
    ...
    for folder1 in os.listdir(img_folder):
        ...
        _, H, W = images.shape
        f1,precision,recall = slideCropF1(images, heatmap,num_class)
